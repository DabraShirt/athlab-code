...
class MLflowTracker:
    ...
    def start_run(self, run_name: str, nested: bool = False):
        ...
        """
        active_run = mlflow.start_run(experiment_id=self.experiment_id, run_name=run_name, nested=nested)
    ...
    def log_model(self, model, name: str, data=None):
        ...
        if data is not None:
            ...
            sample_input = {
                "x_dict": {str(k): v[:1].detach().cpu().numpy() for k, v in data.x_dict.items()},
                "edge_index_dict": {str(k): v[:, :1].detach().cpu().numpy() for k, v in data.edge_index_dict.items()},
            }
            ...
            registered_model_name = f"{self.experiment_name}_{name}"
            mlflow.pytorch.log_model(model, artifact_path=name, registered_model_name=registered_model_name)
